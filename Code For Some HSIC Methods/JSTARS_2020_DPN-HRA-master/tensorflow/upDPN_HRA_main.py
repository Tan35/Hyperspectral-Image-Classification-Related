...
# load the data
    print (150*'*')
    HU2012 = sio.loadmat('datasets/Houston2012training_gt.mat.mat')
    ...
    prototypes = construct_center(features, classes_num, 1)
    
    loss1 = dce_loss(features, labels, prototypes, FLAGS.temp)
    loss2 = dis_loss(features, labels, prototypes)
    loss = loss1 + FLAGS.weight_dis * loss2

    eval_correct = evaluation(features, labels, prototypes)
    train_op = training(loss, lr)
