...
def get_mask(target, train_prop, val_prop, save_dir=None):
    assert train_prop+val_prop <= 1 # 断言train_prop和val_prop的和不超过1，确保训练集和验证集的比例合理。
    train_mask = np.zeros([target.shape[0], target.shape[1]]) # 创建一个与target具有相同形状的全零数组作为训练集掩码。
    val_mask = train_mask.copy() # 创建一个与训练集掩码相同形状的验证集掩码，初始值也为全零。
    test_mask = train_mask.copy() # 创建一个与训练集掩码相同形状的测试集掩码，初始值也为全零。

    for i in range(1, int(target.max())+1):
        indx = np.argwhere(target == i) #找到地物类别 1-16的每个对应的样本索引，len(indx)则找到地物类别 1-16的每个对应的样本数
        train_num = max(round(train_prop*len(indx)), 3) 
        val_num = max(round(val_prop*len(indx)), 3)
        # round 用于将一个数值四舍五入为最接近的整数或指定小数位数的精度
        # max 是一个内置函数，用于返回一组数值或可迭代对象中的最大值。
        #这段代码的目的是计算训练集的样本数量，并确保至少有3个样本用于训练。
        # 如果根据训练集比例计算得到的样本数量小于3，那么max()函数会返回3作为train_num的值，以确保训练集至少包含3个样本。
        # 验证集同理

        np.random.shuffle(indx) # 打乱indx的顺序
        train_indx = indx[: train_num] # 从打乱后的indx中取前train_num个作为训练集的索引。
        val_indx = indx[train_num:train_num+val_num] # 从打乱后的indx中取接下来的val_num个作为验证集的索引。
        test_indx = indx[train_num+val_num:] # 从打乱后的indx中取剩余的索引作为测试集的索引

        train_mask[train_indx[:, 0], train_indx[:, 1]] = 1 #这行代码的作用是将训练集中的样本在train_mask中对应的位置设置为1，以标记这些样本属于训练集。
        val_mask[val_indx[:, 0], val_indx[:, 1]] = 1 # 这行代码的作用是将验证集中的样本在val_mask中对应的位置设置为1，以标记这些样本属于验证集。
        test_mask[test_indx[:, 0], test_indx[:, 1]] = 1 #这行代码的作用是将测试集中的样本在test_mask中对应的位置设置为1，以标记这些样本属于测试集。
...
def get_each_class_num(target, total_num, min_num=3):  # 固定总的训练样本
    ...
    assert total_num >= (class_num * min_num)   # 每类至少3个样本 200>16*3

    target_count = target[target != 0]  # 所有有标签的样本个数  去除背景
    num = float(len(target_count)) # IN: 10249.0
    num_i_ = pd.value_counts(target_count).sort_index().values  # 每一类的总样本数
    # 这段代码使用了Pandas库中的value_counts函数对target_count进行统计，
    # 并在结果上应用了sort_index函数。然后，通过访问values属性，提取了每个类别的总样本数。
    # num_i_是一个包含每个类别的总样本数的NumPy数组，数组的顺序与类别的索引顺序相对应。
    num_i = np.floor(num_i_ / num * total_num).astype(np.int) # eg: IN 第一类 46/10249 * 200，每一类所占总样本比例乘 200 
    num_i[num_i < min_num] = min_num # 保证每一类最少 3 个样本

    if num_i.sum() > total_num: # 如果抽取的总样本数 > total_num(即 200)
        i = 0
        ...
        # 通过这个循环，代码逐个减少num_i数组中的元素，
        # 直到总和等于total_num为止。这样可以确保每个类别的样本数量权重被调整到满足总样本数量要求。
...
def get_fixed_number_masks(target, train_num, val_num, save_dir=None):
    ...
    train = get_each_class_num(target, train_num) # train_num=200
    val = get_each_class_num(target, val_num) # val_num=300
